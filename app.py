from flask import Flask, request, jsonify
from flask_cors import CORS
import os
import json
from datetime import datetime
import logging
from werkzeug.exceptions import BadRequest
import cv2
import numpy as np
import tempfile
from werkzeug.utils import secure_filename

# Import our custom modules
from gesture_recognition import recognize_gesture, detect_emergency_gestures, get_gesture_recognizer, GestureResult
import base64
import cv2
import numpy as np
from situation_analysis import analyze_situation_from_data, get_safety_recommendations

# Initialize Flask app
app = Flask(__name__)
CORS(app)  # Enable CORS for all routes

# Configure logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# Configuration
app.config['MAX_CONTENT_LENGTH'] = 100 * 1024 * 1024  # 100MB max file size for videos
app.config['JSON_SORT_KEYS'] = False

@app.errorhandler(413)
def too_large(e):
    return jsonify({"error": "File too large. Maximum size is 100MB."}), 413

@app.errorhandler(400)
def bad_request(e):
    return jsonify({"error": "Bad request. Please check your input data."}), 400

@app.errorhandler(500)
def internal_error(e):
    return jsonify({"error": "Internal server error. Please try again later."}), 500

@app.route('/', methods=['GET'])
def home():
    """Serve the frontend HTML file"""
    try:
        # Look for index.html in the current directory
        frontend_path = os.path.join(os.path.dirname(__file__), 'index.html')
        with open(frontend_path, 'r', encoding='utf-8') as f:
            return f.read(), 200, {'Content-Type': 'text/html'}
    except FileNotFoundError:
        return jsonify({
            "message": "Smart India Hackathon - Safety & Gesture Recognition API",
            "status": "running",
            "version": "1.0.0",
            "timestamp": datetime.now().isoformat(),
            "endpoints": {
                "gesture_recognition": "/api/recognize-gesture",
                "situation_analysis": "/api/analyze-situation",
                "combined_analysis": "/api/analyze-complete",
                "safety_tips": "/api/safety-tips",
                "health": "/health"
            }
        })

@app.route('/health', methods=['GET'])
def health_check():
    """Simple health check"""
    return jsonify({
        "status": "healthy",
        "timestamp": datetime.now().isoformat()
    })

@app.route('/api/recognize-gesture', methods=['POST'])
def recognize_gesture_endpoint():
    """Endpoint for gesture recognition from image"""
    try:
        # Validate request
        if not request.json:
            return jsonify({"error": "No JSON data provided"}), 400
        
        # Get image data
        image_data = request.json.get('image')
        if not image_data:
            return jsonify({"error": "No image data provided"}), 400
            
        # Validate image data format
        if isinstance(image_data, str):
            if not image_data.strip():
                return jsonify({"error": "Empty image data provided"}), 400
        else:
            return jsonify({"error": "Invalid image data format"}), 400
        
        # Process gesture recognition
        logger.info("Processing gesture recognition request")
        result = recognize_gesture(image_data)
        
        if not isinstance(result, GestureResult):
            logger.error("Gesture recognition failed")
            return jsonify({'success': False, 'error': 'Gesture recognition failed'}), 400
        
        # Format response for new gesture recognition system
        response_data = {
            'success': True,
            'gestures': [],
            'emergency_detected': result.is_emergency,
            'hands_detected': 1 if result.hand_landmarks else 0,
            'confidence': result.confidence,
            'timestamp': datetime.now().isoformat(),
            'processing_time': "<1s"
        }
        
        # Process the detected gesture
        if result.hand_landmarks:
            gesture_info = {
                'gesture': result.gesture_type.value,
                'confidence': result.confidence,
                'emergency': result.is_emergency,
                'description': result.description,
                'handedness': result.hand_landmarks.handedness
            }
            
            # Add bounding box if available
            if result.hand_landmarks.bounding_box:
                bbox = result.hand_landmarks.bounding_box
                gesture_info['bbox'] = {
                    'x': bbox[0],
                    'y': bbox[1],
                    'width': bbox[2],
                    'height': bbox[3]
                }
            
            response_data['gestures'].append(gesture_info)
        
        logger.info(f"Gesture recognition completed. Found {response_data['hands_detected']} hands")
        return jsonify(response_data)
        
    except Exception as e:
        logger.error(f"Gesture recognition endpoint error: {str(e)}")
        return jsonify({"error": f"Processing failed: {str(e)}"}), 500

@app.route('/api/analyze-situation', methods=['POST'])
def analyze_situation():
    """Endpoint for situation analysis from image"""
    try:
        # Validate request
        if not request.json:
            return jsonify({"error": "No JSON data provided"}), 400
        
        # Get image data
        image_data = request.json.get('image')
        if not image_data:
            return jsonify({"error": "No image data provided"}), 400
            
        # Validate image data format
        if isinstance(image_data, str):
            if not image_data.strip():
                return jsonify({"error": "Empty image data provided"}), 400
        else:
            return jsonify({"error": "Invalid image data format"}), 400
        
        # Optional: Get additional context data
        location_data = request.json.get('location')
        gesture_data = request.json.get('gesture_context')
        
        # Process situation analysis
        logger.info("Processing situation analysis request")
        result = analyze_situation_from_data(image_data, gesture_data, location_data)
        
        if "error" in result:
            logger.error(f"Situation analysis error: {result['error']}")
            return jsonify(result), 400
        
        # Add safety recommendations
        if "risk_assessment" in result and "risk_level" in result["risk_assessment"]:
            risk_level = result["risk_assessment"]["risk_level"]
            result["safety_tips"] = get_safety_recommendations(risk_level)
        
        logger.info(f"Situation analysis completed. Risk level: {result.get('risk_assessment', {}).get('risk_level', 'unknown')}")
        return jsonify(result)
        
    except Exception as e:
        logger.error(f"Situation analysis endpoint error: {str(e)}")
        return jsonify({"error": f"Processing failed: {str(e)}"}), 500

@app.route('/api/analyze-complete', methods=['POST'])
def analyze_complete():
    """Endpoint for combined gesture recognition and situation analysis"""
    try:
        # Validate request
        if not request.json:
            return jsonify({"error": "No JSON data provided"}), 400
        
        # Get image data
        image_data = request.json.get('image')
        if not image_data:
            return jsonify({"error": "No image data provided"}), 400
            
        # Validate image data format
        if isinstance(image_data, str):
            if not image_data.strip():
                return jsonify({"error": "Empty image data provided"}), 400
        else:
            return jsonify({"error": "Invalid image data format"}), 400
        
        # Optional: Get additional context data
        location_data = request.json.get('location')
        
        logger.info("Processing complete analysis request")
        
        # Step 1: Gesture recognition
        try:
            # Process gesture recognition with new system
            result = recognize_gesture(image_data)
            
            if not isinstance(result, GestureResult):
                gesture_result = {
                    "error_info": "Gesture recognition failed",
                    "gestures": [],
                    "emergency_detected": False,
                    "partial_results": True
                }
            else:
                # Format response for new gesture recognition system
                gesture_result = {
                    'success': True,
                    'gestures': [],
                    'emergency_detected': result.is_emergency,
                    'hands_detected': 1 if result.hand_landmarks else 0,
                    'confidence': result.confidence
                }
                
                # Create gesture info from the result
                if result.hand_landmarks:
                    gesture_info = {
                        'gesture': result.gesture_type.value,
                        'confidence': result.confidence,
                        'emergency': result.is_emergency,
                        'description': result.description,
                        'handedness': result.hand_landmarks.handedness
                    }
                    
                    # Add bounding box
                    bbox = result.hand_landmarks.bounding_box
                    gesture_info['bbox'] = {
                        'x': bbox[0],
                        'y': bbox[1],
                        'width': bbox[2],
                        'height': bbox[3]
                    }
                    
                    gesture_result['gestures'].append(gesture_info)
                    
        except Exception as gesture_error:
            logger.error(f"Gesture recognition exception: {str(gesture_error)}")
            import traceback
            traceback.print_exc()
            # Continue with partial results
            gesture_result = {
                "error_info": f"Gesture recognition failed: {str(gesture_error)}",
                "gestures": [],
                "emergency_detected": False,
                "partial_results": True
            }
        
        # Step 2: Situation analysis with gesture context
        try:
            situation_result = analyze_situation_from_data(image_data, gesture_result, location_data)
            
            if "error" in situation_result:
                logger.error(f"Situation analysis error in complete analysis: {situation_result['error']}")
                # Continue with partial results
                situation_result = {
                    "error_info": f"Situation analysis encountered an error: {situation_result['error']}",
                    "risk_assessment": {"risk_level": "unknown"},
                    "partial_results": True
                }
        except Exception as situation_error:
            logger.error(f"Situation analysis exception: {str(situation_error)}")
            import traceback
            traceback.print_exc()
            # Continue with partial results
            situation_result = {
                "error_info": f"Situation analysis encountered an error: {str(situation_error)}",
                "risk_assessment": {"risk_level": "unknown"},
                "partial_results": True
            }
        
        # Combine results
        combined_result = {
            "timestamp": datetime.now().isoformat(),
            "analysis_type": "complete",
            "gesture_recognition": gesture_result,
            "situation_analysis": situation_result,
            "overall_assessment": {
                "emergency_detected": False,
                "risk_level": situation_result.get("risk_assessment", {}).get("risk_level", "unknown"),
                "immediate_action_required": False
            }
        }
        
        # Check for emergency conditions
        emergency_gestures = gesture_result.get("emergency_signals", [])
        high_risk = situation_result.get("risk_assessment", {}).get("risk_level") == "high"
        situation_emergency = situation_result.get("emergency_detected", False)
        
        # Check for help gesture specifically
        help_gesture_detected = False
        if "gestures" in gesture_result:
            for gesture in gesture_result.get("gestures", []):
                if gesture.get("gesture") == "help" or gesture.get("name") == "Help Gesture" or gesture.get("message") == "help gesture detected" or gesture.get("emergency", False):
                    help_gesture_detected = True
                    emergency_gestures.append("Help gesture detected")
                    
                    # Add emergency analysis to the response if not already present
                    if "emergency_analysis" not in combined_result:
                        combined_result["emergency_analysis"] = {
                            "emergency_detected": True,
                            "emergency_gestures": []
                        }
                    
                    combined_result["emergency_analysis"]["emergency_gestures"].append({
                        "type": gesture.get("name", "Help Gesture"),
                        "message": "help gesture detected",
                        "confidence": gesture.get("confidence", 0.9)
                    })
                    break
        
        if emergency_gestures or high_risk or situation_emergency or help_gesture_detected:
            combined_result["overall_assessment"]["emergency_detected"] = True
            combined_result["overall_assessment"]["immediate_action_required"] = True
            
            # Compile emergency response
            emergency_actions = []
            if emergency_gestures:
                emergency_actions.extend(["Emergency gesture detected", "Contact emergency services"])
            if help_gesture_detected:
                emergency_actions.extend(["Help gesture detected", "Immediate assistance required"])
            if high_risk:
                emergency_actions.extend(["High risk situation identified", "Move to safer location"])
            
            combined_result["emergency_response"] = {
                "urgency": "high",
                "recommended_actions": emergency_actions,
                "emergency_contacts": {
                    "police": "100",
                    "medical": "108",
                    "fire": "101",
                    "women_helpline": "1091"
                }
            }
        
        # Add safety recommendations
        risk_level = situation_result.get("risk_assessment", {}).get("risk_level", "medium")
        combined_result["safety_recommendations"] = get_safety_recommendations(risk_level)
        
        logger.info(f"Complete analysis finished. Emergency: {combined_result['overall_assessment']['emergency_detected']}")
        return jsonify(combined_result)
        
    except Exception as e:
        logger.error(f"Complete analysis endpoint error: {str(e)}")
        return jsonify({"error": f"Processing failed: {str(e)}"}), 500

@app.route('/api/safety-tips', methods=['GET'])
def safety_tips():
    """Endpoint to get safety tips based on risk level"""
    try:
        risk_level = request.args.get('risk_level', 'medium')
        
        if risk_level not in ['very_low', 'low', 'medium', 'high']:
            return jsonify({"error": "Invalid risk level. Use: very_low, low, medium, high"}), 400
        
        tips = get_safety_recommendations(risk_level)
        
        return jsonify({
            "risk_level": risk_level,
            "safety_tips": tips,
            "timestamp": datetime.now().isoformat()
        })
        
    except Exception as e:
        logger.error(f"Safety tips endpoint error: {str(e)}")
        return jsonify({"error": f"Failed to get safety tips: {str(e)}"}), 500

@app.route('/api/emergency', methods=['POST'])
def emergency_alert():
    """Endpoint for emergency alerts"""
    try:
        if not request.json:
            return jsonify({"error": "No JSON data provided"}), 400
        
        alert_type = request.json.get('type', 'general')
        location = request.json.get('location')
        message = request.json.get('message', 'Emergency alert triggered')
        
        # Log emergency alert
        logger.warning(f"EMERGENCY ALERT: Type: {alert_type}, Location: {location}, Message: {message}")
        
        # In a real implementation, this would trigger actual emergency services
        response = {
            "alert_id": f"ALERT_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            "status": "received",
            "timestamp": datetime.now().isoformat(),
            "type": alert_type,
            "message": "Emergency alert has been logged and processed",
            "next_steps": [
                "Alert has been logged in the system",
                "If this is a real emergency, contact local emergency services",
                "Emergency contacts: Police (100), Medical (108), Fire (101)"
            ]
        }
        
        return jsonify(response)
        
    except Exception as e:
        logger.error(f"Emergency alert endpoint error: {str(e)}")
        return jsonify({"error": f"Failed to process emergency alert: {str(e)}"}), 500

@app.route('/api/analyze-video', methods=['POST'])
def analyze_video():
    """Endpoint for video analysis and gesture recognition"""
    try:
        # Check if video file is present
        if 'video' not in request.files:
            return jsonify({"error": "No video file provided"}), 400
        
        video_file = request.files['video']
        if video_file.filename == '':
            return jsonify({"error": "No video file selected"}), 400
        
        # Validate file type
        allowed_extensions = {'mp4', 'avi', 'mov', 'mkv', 'webm'}
        file_extension = video_file.filename.rsplit('.', 1)[1].lower() if '.' in video_file.filename else ''
        if file_extension not in allowed_extensions:
            return jsonify({"error": f"Unsupported file type. Allowed: {', '.join(allowed_extensions)}"}), 400
        
        # Create temporary file
        with tempfile.NamedTemporaryFile(delete=False, suffix=f'.{file_extension}') as temp_file:
            video_file.save(temp_file.name)
            temp_video_path = temp_file.name
        
        try:
            # Process video
            logger.info(f"Processing video: {video_file.filename}")
            start_time = datetime.now()
            
            # Open video with OpenCV
            cap = cv2.VideoCapture(temp_video_path)
            if not cap.isOpened():
                return jsonify({"error": "Failed to open video file"}), 400
            
            # Get video properties
            fps = cap.get(cv2.CAP_PROP_FPS)
            total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
            duration = total_frames / fps if fps > 0 else 0
            
            # Process frames (sample every 30 frames or 1 second)
            frame_interval = max(1, int(fps)) if fps > 0 else 30
            frame_results = []
            gesture_counts = {}
            emergency_detected = False
            frames_with_gestures = 0
            
            frame_number = 0
            while True:
                ret, frame = cap.read()
                if not ret:
                    break
                
                # Process every nth frame
                if frame_number % frame_interval == 0:
                    # Convert frame to base64 for gesture recognition
                    _, buffer = cv2.imencode('.jpg', frame)
                    frame_base64 = buffer.tobytes()
                    import base64
                    frame_base64_str = base64.b64encode(frame_base64).decode('utf-8')
                    
                    # Analyze frame
                    try:
                        # Recognize gesture in frame
                        frame_result = recognize_gesture(frame_base64_str)
                        
                        if not isinstance(frame_result, GestureResult) or not frame_result.hand_landmarks:
                            continue
                            
                    except Exception as e:
                        logger.warning(f"Failed to analyze frame {frame_number}: {str(e)}")
                        continue
                    
                    if frame_result.hand_landmarks:
                        frames_with_gestures += 1
                        gesture_info = {
                            'gesture': frame_result.gesture_type.value,
                            'confidence': frame_result.confidence,
                            'emergency': frame_result.is_emergency,
                            'description': frame_result.description,
                            'handedness': frame_result.hand_landmarks.handedness
                        }
                        
                        frame_info = {
                            'frame_number': frame_number,
                            'timestamp': frame_number / fps if fps > 0 else 0,
                            'gestures': [gesture_info]
                        }
                        frame_results.append(frame_info)
                        
                        # Count gestures
                        gesture_name = frame_result.gesture_type.value
                        gesture_counts[gesture_name] = gesture_counts.get(gesture_name, 0) + 1
                    
                    # Check for emergency gestures
                    if frame_result.is_emergency:
                        emergency_detected = True
                
                frame_number += 1
                
                # Limit processing to avoid timeout (max 100 frames)
                if len(frame_results) >= 100:
                    break
            
            cap.release()
            
            # Calculate processing time
            end_time = datetime.now()
            processing_time = (end_time - start_time).total_seconds()
            
            # Determine most common gesture
            most_common_gesture = max(gesture_counts.keys(), key=gesture_counts.get) if gesture_counts else 'None'
            
            # Perform situation analysis
            situation_data = {
                'gesture_recognition': {
                    'gestures_detected': len(gesture_counts) > 0,
                    'gesture_types': list(gesture_counts.keys()),
                    'emergency_gestures': emergency_detected
                },
                'video_analysis': {
                    'duration': duration,
                    'total_frames': total_frames,
                    'processed_frames': len(frame_results)
                }
            }
            
            situation_result = analyze_situation_from_data(situation_data)
            # Extract risk level from nested structure
            if isinstance(situation_result, dict) and 'risk_assessment' in situation_result:
                risk_level = situation_result['risk_assessment'].get('risk_level', 'medium')
            else:
                risk_level = 'medium'
            safety_tips = get_safety_recommendations(risk_level)
            
            # Prepare response
            response = {
                'success': True,
                'video_info': {
                    'filename': secure_filename(video_file.filename),
                    'duration': round(duration, 2),
                    'fps': round(fps, 2),
                    'total_frames': total_frames
                },
                'processing_info': {
                    'processed_frames': len(frame_results),
                    'processing_time': f"{processing_time:.2f}s",
                    'frame_interval': frame_interval
                },
                'gesture_summary': {
                    'frames_with_gestures': frames_with_gestures,
                    'unique_gestures': len(gesture_counts),
                    'gesture_counts': gesture_counts,
                    'most_common_gesture': most_common_gesture
                },
                'emergency_analysis': {
                    'emergency_detected': emergency_detected,
                    'risk_level': risk_level
                },
                'frame_results': frame_results[:10],  # Return first 10 results
                'safety_recommendations': safety_tips,
                'timestamp': datetime.now().isoformat()
            }
            
            logger.info(f"Video analysis completed in {processing_time:.2f}s")
            return jsonify(response)
            
        finally:
            # Clean up temporary file
            try:
                os.unlink(temp_video_path)
            except OSError:
                pass
    
    except Exception as e:
        logger.error(f"Video analysis error: {str(e)}")
        return jsonify({"error": f"Failed to analyze video: {str(e)}"}), 500



if __name__ == '__main__':
    # Get port from environment variable or default to 5000
    port = int(os.environ.get('PORT', 5000))
    
    # Get debug mode from environment variable
    debug_mode = os.environ.get('FLASK_DEBUG', 'False').lower() == 'true'
    
    logger.info(f"Starting Smart India Hackathon API server on port {port}")
    logger.info(f"Debug mode: {debug_mode}")
    
    # Run the Flask app
    app.run(
        host='0.0.0.0',
        port=port,
        debug=debug_mode,
        threaded=True
    )
